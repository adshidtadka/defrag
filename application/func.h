...
int genDemands(int load) {
    ...
    ofs_input << "expired number per 1 sec and inter arrival time_slot_now:=" << expired_num << ", " << inter_arr
              << endl;
    ...
    for (int i = 0; i < Constant::REQ_NUM; ++i) {
        ...
        ofs_input << i << ": " << source[i] << " " << dest[i] << " " << lp_size[i] << " " << t_req_event[i] << " "
                  << t_hold_event[i] << endl;
    }
    ...
    while (realMov) {
        ...
        while (cur != NULL) {
            ...
            if (realcheck != realOp) {
                ...
                if ((time_slot_now + ret_time >= fin_time || ret_time >= Constant::DEFRAG_TOTAL_TIME_MAX) ||
                    eventQueue.empty()) {
                    return 0;
                }
            }
...
int retuneDown() {
    ...
    while (index1 < Constant::CAPACITY - 1 && k++ < Constant::INF) {            //Sweep the spectrum
        ...
        if (nextEvent.type == 0 &&
            (time_slot_now + ret_time >= nextEvent.time || ret_time >= Constant::DEFRAG_TOTAL_TIME_MAX)) {
        ...
        for (i = 0; i < b; i++) {
            for (j = 0; j < Constant::LINK_NUM; j++) {
                if (path_prim[j][lp] == 1 && spec[index + i][j] == 0) {
                    cout << "path_prim[" << j << "][" << lp << "] = " << path_prim[j][lp] << ", spec[" << index + i
                         << "][" << j << "] = " << spec[index + i][j] << endl;
            ...
            for (i = 0; i < b; i++) {
                for (j = 0; j < Constant::LINK_NUM; j++) {
                    if (path_back[j][lp] == 1 && spec[index + i][j] == 0) {
                        cout << "path_back[" << j << "][" << lp << "] = " << path_back[j][lp] << ", spec[" << index + i
                             << "][" << j << "] = " << spec[index + i][j] << endl;
